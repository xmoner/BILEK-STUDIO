...
def main(page: ft.Page):
    ...
    def create_items_view():
        ...
        def update_items_list(search_term=""):
            ...
            for item in items:
                d = item['id']  # Access the 'id' key
                dd = item['name']   # Access the 'name' key
                ddd = item['price'] 
                            ...
                            content=ft.Column(
                                [
                                    ft.ListTile(
                                        ...
                                        title=ft.Text(dd, size=16),
                                        subtitle=ft.Text(f"Price: {float(ddd)} JD"),
                                    ),
                                    ft.Row(
                                        [
                                            ft.IconButton(
                                                ...
                                                on_click=lambda e, item_id=d: edit_item_dialog(item_id)
                                            ),
                                            ft.IconButton(
                                                icon=ft.Icons.DELETE,
                                                icon_color=ft.Colors.RED,
                                                on_click=lambda e, item_id=d: delete_item_dialog(item_id)
                                            ),
                                        ],
        ...
        def add_item_dialog():
            ...
            def save_new_item(e):
                ...
                try:
                    ...
                    database.add_menu_item(name_field.value, price)
                    close_dialog(dia)
            ...
            save_button=ft.TextButton("Save", on_click=save_new_item)
            dia = ft.AlertDialog(
                title=ft.Text("Add New Item"),
                content=ft.Column([name_field, price_field], tight=True),
                actions=[
        ...
        def edit_item_dialog(item_id):
            ...
            if item:
                ...
                options=[
                    ft.dropdown.Option(
                        key=str(cat['id_category']),  # Use 'id_category' as the key
                        text=cat['name_category']  # Use 'name_category' as the text
                    ) for cat in categories
                ],
                value=str(item['id_category']) if item and 'id_category' in item else None,  # Set current category
                width=300
            )

                def save_edit(e):
                    save_edit_button.disabled=True
                ...
                save_edit_button=ft.TextButton("Save", on_click=save_edit)        
                dia = ft.AlertDialog(
        ...
        def show_row_details(row_data):
            ...
            def delete_row(e):
                ...
                try:
                    ...
                    page.update()    
            delete_button = ft.TextButton("Delete", on_click=delete_row)
            dia = ft.AlertDialog(
    ...
    def create_home_view(is_admin=False):
        ...
        def new_check(e):
            if len(item_list.controls) > 0:
                ...
                error_text.value = "There is no item in the cart."    
        ...
        def show_bill_dialog(bill_summary):
            ...
            bill_dialog = ft.AlertDialog(
                title=ft.Text(f"Bi55 Summary - {timestamp}",size=11, weight=ft.FontWeight.BOLD),
                content=ft.Text(bill_summary),
                actions=[
                    ft.TextButton("Close", on_click=lambda e: close_dialog(e, bill_dialog))
                ]
            )
        ...
        for item in menu_items:
            ...
            content_width = len(max(name, f"{price:.2f} JD", key=len)) * 8  # Approximate character width
            
            item_container = ft.Stack(
                [
                    ft.Container(
                        content=ft.Column(
                            [
                        ...
                        border_radius=10,
                        on_click=add_item(name, price),
                        ink=True,
                        padding=ft.padding.all(10),
                        # width=max(content_width, ),  # Set minimum width
                        height=None,  # Let height adjust automatically
                    ),
                ],
                expand=False,
            )
            
            menu_grid.controls.append(item_container)
        ...
        for category in categories:
            ...
            cat_name = category['name_category']
    ...
    "/",drawer= ft.NavigationDrawer(on_change=handle_change,controls=[
       ...
       on_click=lambda _: page.go("/")),], alignment=ft.MainAxisAlignment.CENTER,
        ...
        ]),controls=[*([ft.AppBar(title = ft.Row(controls = [ft.Text("P.O.S System",
